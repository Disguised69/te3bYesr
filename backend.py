...
from webcamInterface import gen_frames , capture_webcam_image, last_frame
...
UPLOAD_FOLDER = 'faces'
def saveImage(user_folder):
    try:
        image = np.copy(last_frame["x"])
    except Exception as e:
...
def capture_image():
    ...
    my_thread = threading.Thread(target=saveImage, args=(user_folder,))
    # Start the thread
    #my_thread.start()
    return jsonify({'status': 'success', 'message': 'Face captured and saved.'}),200
...
@app.route('/api/v1/identify_person', methods=['POST'])
def identify_person():
    persons = Person.query.all()
    if persons:
        ...
        with open('log.csv', 'a', newline='') as file:
            ...
            if not file_exists or os.stat('log.csv').st_size == 0:
                ...
                writer.writerow(['Timestamp', 'Person Identified', 'Distance', 'Probability'])

            # Log the data
            timestamp = datetime.datetime.now().isoformat()
            writer.writerow([timestamp, name, distance, knn_proba])

        if distance < distance_threshold:
            print({'found': True, 'person': name, 'proba': knn_proba, 'distance': distance})
            return jsonify({'found': True, 'person': name, 'proba': float(knn_proba), 'distance': float(distance)}), 200
        else:
            print({'found': False, 'person': name, 'proba': knn_proba, 'distance': distance})
            return jsonify({'found': False, 'person': name, 'proba': float(knn_proba), 'distance': float(distance)}), 404
...
def video_feed():
    # Return the mimetype as multipart/x-mixed-replace to stream the content
